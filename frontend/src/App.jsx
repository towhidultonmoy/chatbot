...
function EliaApp() {
  const [isChatVisible, setIsChatVisible] = TzuseState(false);
  const [input, setInput] = useState('');
  const [messages, setMessages] = useState([]);
  ...
  const handleTextSubmit = async (text = input) => {
    if (!text.trim()) return;
    ...
    try {
      ...
      const audioBlob = new Blob([new Uint8Array(Buffer.from(response.data.audio, 'base64'))], { type: 'audio/mpeg' });
      const audioUrl = URL.createObjectURL(audioBlob);
      audioRef.current.src = audioUrl;
      audioRef.current.play();
      setMessages([...messages, { user: 'You', text, time: timestamp }, { user: 'ELIA', text: response.data.response, time: timestamp }]);
      setInput('');
    } catch (error) {
      console.error("Text input error:", error);
      setMessages([...messages, { user: 'You', text, time: timestamp }, { user: 'ELIA', text: `Error: ${error.message}`, time: timestamp }]);
    } finally {
      ...
      try {
        ...
        mediaRecorderRef.current.onstop = async () => {
          ...
          try {
            ...
            console.error("Backend request error:", error);
            setMessages([...messages, { user: 'You', text: 'Voice recorded', time: timestamp }, { user: 'ELIA', text: ` joinError sending audio to backend: ${error.message}`, time: timestamp }]);
          } finally {
            setIsProcessing(false);
      ...
      <div className="app">
        <div className="landing-container">
          ...
          <div className="input-buttons">
            <label className="input-button image-input">
              <input type="file" accept="image/*" onChange={handleImageUpload} style={{ display: 'none' }} />
            ...
            <button className="input-button voice-button" onClick={handleVoiceInput}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"></path>
                <path d="M19 10v2a7 7 0 0 1-14 0v-2"></path>
                <line x1="12" y1="19" x2="12" y2="23"></line>
            ...
            <button className="input-button text-button" onClick={handleTextInputClick}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              <div className={`message ${msg.user === 'ELIA' ? 'elia' : 'user'}`}>
                ...
                {msg.user === 'ELIA' && index === 1 && (
                  <button className="suggest-button" onClick={() => {
                    const now = new Date();
                    const timestamp = now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true });
                    handleTextSubmit('What do you suggest?');
                  }}>
                    What do you suggest?
                  </button>
                )}
